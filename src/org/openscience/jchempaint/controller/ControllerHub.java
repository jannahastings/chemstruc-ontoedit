...
import org.openscience.jchempaint.renderer.generators.FragmentGroupGenerator;
...
public class ControllerHub implements IMouseEventRelay, IChemModelRelay {
    ...
    private FragmentGroupHandler fragmentGroupHandler;
    ...
    public void unsetFragmentGroupHandler(){
        this.fragmentGroupHandler=null;
        for (IGenerator generator : this.getRenderer().getGenerators()) {
            if (generator instanceof FragmentGroupGenerator) {
                ((FragmentGroupGenerator) generator).setFragmentGroup(null);
            }
        }
        if (chemModel.getMoleculeSet()!=null)
            for (IAtomContainer atc : chemModel.getMoleculeSet().atomContainers()) {
    ...
    public FragmentGroupHandler getFragmentGroupHandler() {
        return fragmentGroupHandler;
    }

    public void setFragmentGroupHandler(FragmentGroupHandler fragmentGroupHandler) {
        this.fragmentGroupHandler = fragmentGroupHandler;
        if (fragmentGroupHandler != null) {
            for (IGenerator generator : renderer.getGenerators())
                if (generator instanceof FragmentGroupGenerator) {
                    ((FragmentGroupGenerator) generator).setFragmentGroup(fragmentGroupHandler.getFragmentGroup());
                }
        }
    }
}
